...
model = tf.keras.models.load_model('multilabel_model.h5')  # Pastikan model sudah disimpan sebelumnya
with open('tokenizer.pkl', 'rb') as f:
    tokenizer = pickle.load(f)

# Load data wisata (pastikan file java.xlsx sudah ada)
...
def recommend_places(user_province, user_labels, user_description, df, top_n=50, threshold=0.10):
    ...
    # Terapkan threshold untuk filter berdasarkan cosine similarity
...
def recommend():
    ...
    threshold = data.get('threshold', 0.10)  # Default threshold set to 0.10
